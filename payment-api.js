const PAYMENT_API = {
    // ============================================================
    // LZPEDIA API INTEGRATION - V3 (DIRECT + PROXY FALLBACK)
    // ============================================================

    // API Key langsung (untuk direct call, tapi jangan expose di production!)
    // Di production, gunakan proxy PHP
    apiKey: 'LXZ_015d8a759df64d48',
    baseUrl: 'https://app.lzpedia.my.id/api',

    // Proxy URL (harus di-upload ke server yang sama)
    backendUrl: window.location.origin + '/lzproxy.php',

    // Mode: 'proxy' (recommended) atau 'direct' (hanya untuk test)
    mode: 'proxy',
    ...
    async createInvoice(amount) {
        try {
            ...
            // Ini hanya untuk test/debug. Di production, proxy WAJIB berfungsi!
...
window.createInvoice = async function(amount) {
    ...
    try {
        ...
        if (result.success && result.invoiceId) {
            ...
            document.getElementById('invoiceTotal').textContent = 'Rp ' + Number(result.total).toLocaleString('id-ID');
            document.getElementById('invoiceFee').textContent = 'Rp ' + Number(result.fee || 0).toLocaleString('id-ID');
            ...
            document.getElementById('invoiceExpiry').textContent = expiryDate.toLocaleString('id-ID');
...
window.openInvoiceDetail = function(invoiceId) {
    ...
    document.getElementById('invoiceTotal').textContent = 'Rp ' + Number(invoice.total || invoice.amount).toLocaleString('id-ID');
    document.getElementById('invoiceFee').textContent = 'Rp ' + Number(invoice.fee || 0).toLocaleString('id-ID');
    document.getElementById('invoiceExpiry').textContent = invoice.expired_at ? new Date(invoice.expired_at).toLocaleString('id-ID') : '-';
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('🔧 Payment System - LZPedia API v3');
...
console.log('✅ payment-api.js v3 Loaded!');
                    rawResponse: text.substring(0, 500)
                };
            }
    ...
    async checkInvoiceStatus(invoiceId) {
        try {
            const url = `${this.backendUrl}?action=status&invoice_id=${invoiceId}`;
...
window.createInvoice = async function(amount) {
    ...
    try {
        ...
        if (result.success && result.invoiceId) {
            ...
            let errorMsg = result.error || 'Gagal membuat invoice';
        ...
        if (qrisPlaceholder) {
            ...
            qrisPlaceholder.innerHTML = `
                <div style="text-align:center;max-width:100%;">
                    <i class="fas fa-exclamation-triangle" style="font-size:3rem;color:var(--red);display:block;margin-bottom:12px;"></i>
                    <p style="color:var(--red);font-weight:700;margin-bottom:8px;font-size:1rem;">Gagal Membuat Invoice</p>
                    <p style="color:var(--text-muted);font-size:0.8rem;margin-bottom:12px;word-break:break-word;">${error.message}</p>
                    <div style="background:rgba(255,255,255,0.05);border-radius:8px;padding:12px;margin-bottom:16px;text-align:left;">
                        <p style="color:var(--text-muted);font-size:0.7rem;margin-bottom:4px;"><strong>Tips Perbaikan:</strong></p>
                        <ol style="color:var(--text-muted);font-size:0.7rem;padding-left:16px;margin:0;">
                            <li>Pastikan file <b>lzproxy.php</b> sudah di-upload ke server</li>
                            <li>Pastikan API Key di lzproxy.php sudah benar</li>
                            <li>Cek console browser (F12) untuk detail error</li>
        ...
        if (qrisWrapper) qrisWrapper.style.display = 'none';
        showToast('❌ Gagal', error.message.substring(0, 100), 'error');
    } finally {
        if (createBtn) {
            createBtn.disabled = false;
            createBtn.innerHTML = '<i class="fas fa-qrcode"></i> Buat Invoice QRIS';
...
window.openInvoiceDetail = function(invoiceId) {
    ...
    document.getElementById('paymentDetails').style.display = 'block';
    document.getKeyById('checkStatusBtn').style.display = 'inline-flex';
...
// ============================================================
document.addEventListener('DOMContentLoaded', function() {
    console.log('🔧 Payment System - LZPedia API Integration v2');
    console.log('🔗 Backend:', PAYMENT_API.backendUrl);
...
console.log('✅ payment-api.js v2 Loaded!');
